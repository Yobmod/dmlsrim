...
def is_real_elements(formula: FormulaType) -> bool:
    ...
    if (
        formula[0].isalpha()
        and not formula[0].isupper()
        and formula[0] not in elem_first_letters
    ):
        return False
...
def _fuse(
    mol1: Mapping[str, int], mol2: Mapping[str, int], w: int = 1
) -> Dict[str, int]:
    """
...
def parse_formula(formula: FormulaType) -> Dict[str, int]:
    ...
    warn_text = (
        colored("Check your formula [", "yellow", attrs=["bold"])
        + colored(formula, "red", attrs=["bold"])
        + colored(
            "] - are all the element symbols correct?\n", "yellow", attrs=["bold"]
        )
    )
    ...
    try:
        if not is_balanced(formula):
            raise ValueError(
                colored("Watch your brackets ![{]$[&?)]}!]", "red", attrs=["bold"])
            )
